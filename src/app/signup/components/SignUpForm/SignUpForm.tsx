...
export const SignUpForm = ({ type }: Props) => {
  ...
  const [registrationNumberValid, setRegistrationNumberValid] =
    useState<InputValidation>("none");
  const [registrationNumberMsg, setRegistrationNumberMsg] =
    useState<string>("");
    ...
    try {
      ...
      if ("FAIL_Message" in res) {
        ...
        if (username.length > 1 && username.match(idRule)) {
          ...
          setCheckIdValid("error");
          setCheckIdValidMsg(
            "20자 이내의 영어 소문자, 대문자, 숫자를 조합해 입력해주세요."
          );
        }
      }
    } catch (err) {
  ...
  const onCheckStoreNameValid = async () => {
    ...
    try {
      const res = await authManage.checkCompanyNumberValid(
        company_registration_number
      );
      if ("FAIL_Message" in res) {
  ...
  const onSubmit = async (data: any) => {
    const formData: BuyerSignupReq = {
      ...
      phone_number:
        "0" + parseInt(data.firstNum) + data.middleNum + data.lastNum,
      name: data.name,
    };
    ...
    if (checkIdValid === "valid" && isAgree) {
      try {
        if (type === "BUYER") {
          ...
          if (
            Object.values(res)[0] === "해당 사용자 전화번호는 이미 존재합니다."
          ) {
